...
| `src/lib/config.ts`, `.env.example` | Where a build points: the Supabase project URL and publishable key, the site's address and the Google web client ID. All public (RLS protects the data); the `NEXT_PUBLIC_*` variables in `.env.example` point a build at your own project instead |
...
Without a `.env.local`, the development server and builds use the live app's Supabase project (`src/lib/config.ts`). To use your own, copy `.env.example` to `.env.local` and fill it in; [Setting it up again from scratch](#setting-it-up-again-from-scratch) has the steps. Sign-in links only come back to addresses listed in Supabase under Authentication → URL Configuration (anything else lands on the project's Site URL), so to sign in on `localhost`, add `http://localhost:3000/**` there.


## How it's set up

- **Code:** this repository, [`raja2102598/gymlog`](https://github.com/raja2102598/gymlog) on GitHub, under the [MIT License](#license). [CONTRIBUTING.md](CONTRIBUTING.md) has how to help and [SECURITY.md](SECURITY.md) how to report a security problem.
...
1. In a Supabase project, run `supabase/schema.sql` (SQL Editor → New query → paste → Run). Then point the build at it: set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (`.env.example` says where: `.env.local` on your machine, the Vercel project's environment variables, and repository variables for the Android workflow), or change the defaults in `src/lib/config.ts`.
2. On Vercel: Add New → Project → import the repo. `vercel.json` sets the build, so the defaults are fine.
3. In Supabase → Authentication → URL Configuration, set the Site URL to the new site address and add the same address followed by `/**` as a redirect URL. For the Android app, set `NEXT_PUBLIC_SITE_URL` to the new address too: its sign-in links land on that address's `app-login.html`.
...
3. **The app:** set `NEXT_PUBLIC_GOOGLE_WEB_CLIENT_ID` to the web client ID (it isn't secret: it's in every Google sign-in link; the secret stays in Supabase), or change the default in `src/lib/config.ts`, and build a new version. The website needs nothing more.
...
### Install
1. On the phone, open the repo's [**Releases**](https://github.com/raja2102598/gymlog/releases) in Chrome and the latest **Gym Log for Android**, and download `gym-log.apk`.
