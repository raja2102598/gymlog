...
**From inside the app:** open **Settings → About → Check for updates**. Found one? **Download and install** fetches it, checks its SHA-256 against what CI published and that it's signed with Gym Log's own key, then hands it to Android's installer. The first time, Android asks you to allow installs from Gym Log; allow it and the app tries installing again by itself as soon as you come back. The app also looks quietly, at most every 12 hours, and shows a small banner when one's ready, linking to the same place. A build with no repository to check (one you built yourself without `GYMLOG_UPDATE_REPO`, below) shows no update controls at all.
