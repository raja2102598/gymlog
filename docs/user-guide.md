...
- **Lifts and sets:** each lift is a card.
  ...
  - **Plates:** tap the stack icon next to a set's kg box for the plates to load on each side, on the lift's bar (the barbell's, the EZ bar's, the trap bar's or the Smith machine's) with your gym's plates, as set in **Settings → My gym → Weights**. It finds the closest loading to the weight with the fewest plates; if the weight can't be made exactly, it says what's left over; a weight under the bar says so instead of a breakdown. A machine's plates go on no bar. A lift done with dumbbells, kettlebells, a cable, bands or bodyweight has no stack icon.
  ...
  - **Warm-up sets:** tap **Warm-up sets**, type a working weight (it suggests the lift's next one), and see 40, 60 and 80 percent of it with rep counts, rounded to what the lift's equipment makes: its bar and pairs of plates, or dumbbells in their step. **Log warm-up sets** logs all three with one tap. They're marked as warm-ups, so they never count toward the lift's planned sets, its tick, or a record. **Remove warm-up sets** takes them off again.
...
- **Adding weight:** when every set of a lift reached the top of its rep range last time, the lift says *Go up to … kg* and the grey numbers switch to the new weight at the bottom of the range. Each lift adds what its equipment goes up by (**Settings → My gym → Weights**: 2 kg for dumbbells, a pair of the smallest plate on a bar, and so on), to a weight that equipment makes, unless you set its own step in the plan; a lift the library doesn't know adds 2.5 kg. That's double progression, the default; a lift can use another rule instead (see **Edit plan**), and its hint names the rule: *linear* adds the step every session each set reaches the bottom of the range, and *% of 1RM* says *Work at … kg*, a percentage of a 1RM you enter, to the nearest weight the lift's equipment makes. With a deload set, once that many sessions in a row fall short of the sets and reps, the lift says *Deload to … kg* instead, taking 10 percent off (or your own percentage), to build back up from.
...
- **Weights**, in My gym: what each bar weighs (barbell 20 kg, EZ bar 10 kg, trap bar 20 kg and the Smith machine's 0 kg, as most are counterbalanced) and the plates your gym has (25, 20, 15, 10, 5, 2.5 and 1.25 kg to start with; list your own separated by commas), for the plates button on a set and warm-up sets; and what dumbbells (2 kg), kettlebells (4 kg), machines and cables (2.5 kg) and bands (5 kg) go up by. A lift goes up by its equipment's step unless it has its own, and the weights suggested for it (*Go up to*, the grey numbers and warm-up sets) round to what that equipment makes, so a dumbbell lift never suggests 23.75 kg.
