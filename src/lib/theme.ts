...
/** The page's ground colour in each theme (tokens.css), for the browser's bar. */
const GROUND = { light: "#eef0f2", dark: "#121417" } as const;
...
export function applyTheme(t: Theme) {
  ...
  for (const m of document.querySelectorAll<HTMLMetaElement>('meta[name="theme-color"]')) {
    ...
    m.content = t === "system" ? m.dataset.system : GROUND[t];
  }
}
